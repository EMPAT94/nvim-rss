...
local feedparser = { _DESCRIPTION = "RSS and Atom feed parser", _VERSION = "feedparser 0.71" }
    ...
    for i, el in ipairs(entry_el:getChildren("*")) do
      ...
      elseif (format_str == "atom" and tag == "link") or (format_str == "rss" and tag == "atom:link") then
        ...
        if link.rel == "enclosure" then
          tinsert(entry.enclosures, { href = link.href, length = el:getAttr("length"), type = el:getAttr("type") })
        end

        -- rss enclosures
      elseif format_str == "rss" and tag == "enclosure" then
        tinsert(entry.enclosures, { url = el:getAttr("url"), length = el:getAttr("length"), type = el:getAttr("type") })
      ...
      elseif (format_str == "atom" and tag == "content") or
          (format_str == "rss" and
              (tag == "body" or tag == "xhtml:body" or tag == "fullitem" or tag == "content:encoded")) then
      ...
      elseif tag == "created" or tag == "atom:created" or tag == "dcterms:created" then
        ...
        -- id
      elseif (format_str == "atom" and tag == "id") or (format_str == "rss" and tag == "guid") then
      ...
      elseif format_str == "atom" and tag == "author" then
        entry.author = (el:getChild("name") or el):getText()
        entry.author_detail = { name = entry.author, email = (el:getChild("email") or blanky):getText() }
...
local function atom_person_construct(person_el, base_uri)
  ...
  if url_el then
    dude.href = resolve(url_el:getText(), rebase(url_el, base_uri))
  end
  return dude
end
...
local function parse_atom(root, base_uri)
  local res = {}
  local feed = { links = {}, contributors = {}, language = root:getAttr("lang") or root:getAttr("xml:lang") }
  local root_base = rebase(root, base_uri)
  res.feed = feed
  res.format = "atom"
  local version = (root:getAttr("version") or ""):lower()
  if version == "1.0" or root:getAttr("xmlns") == "http://www.w3.org/2005/Atom" then
    res.version = "atom10"
...
local function parse_rss(root, base_uri)
  ...
  if not channel then
    return nil, "can't parse that."
  end

  local feed = { links = {}, contributors = {} }
  local res = { feed = feed, format = "rss", entries = {} }
  ...
  for i, el in ipairs(channel:getChildren("*")) do
    ...
    elseif tag == "pubDate" or tag == "dc:date" or tag == "dcterms:modified" then
      ...
      -- author
    elseif tag == "managingEditor" or tag == "dc:creator" or tag == "itunes:author" or tag == "dc:creator" or tag ==
        "dc:author" then
    ...
    elseif tag == "image" or tag == "rdf:image" then
      ...
      if url_el then
        feed.image.href = resolve(url_el:getText(), rebase(url_el, el_base))
      end
...
function feedparser.parse(xml_string, base_url)
  ...
  if not lom then
    return nil, "couldn't parse xml. lxp says: " .. err or "nothing"
  end
